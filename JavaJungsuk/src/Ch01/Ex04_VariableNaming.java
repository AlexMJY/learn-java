package Ch01;

public class Ex04_VariableNaming {
    public static void main(String[] args) {
        // 변수 이름 짓는 법
        // 1.  저장할 값에 어울리는 이름
        // 2. 밑줄(_), 문자(abc), 숫자(123) 사용 가능 (공백 사용 불가)
        // 3. 밑줄 또는 문자로 시작 가능
        // 4. 한 단어 또는 2개 이상 단어의 연속
        // 5. 소문자로 시작, 각 단어의 시작 글자는 대문자로 (첫 단어는 제외)
        // 6. 예약어 사용 불가 (public, static, void, int, double, float ...)

        // 입국 신고서
        String nationality = "Korea";
        String firstName = "주영";
        String lastName = "문";
        String dateOfBirth = "2005-05-13";
        String residential_address = "Hilton";
        String purposeOfVisit = "travel";
        String _flintNumber = "KE447";


        // 크게 이름이 중요하지 않을 때 + 프로그램의 흐름을 위해 사용되는 경우
        int i = 0;
        String s = "";
        String str = "";

        // 절대 변하지 않는 상수는 대문자로
        final String CODE = "KR";
        // CODE = "US"; 에러 발생
    }
}
